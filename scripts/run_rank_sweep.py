...
def main() -> None:
    ...
    parser.add_argument("--config", default="configs/rank_sweep.yaml")
    ...
    for rank in sweep_cfg["ranks"]:
        cfg = copy.deepcopy(base_cfg)
        cfg["model"]["mixer_rank"] = int(rank)
        cfg["train"]["save_dir"] = str(output_root / f"rank_{int(rank):03d}" / "checkpoints")
        cfg_path = output_root / f"rank_{int(rank):03d}" / "config_used.yaml"
